...
impl Widget for CardView {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let rank_suit = Paragraph::new(Line::from(vec![
            Span::styled(
...
impl Widget for CardRowView {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let visible_cards = ((area.width as usize) / (card_width as usize + spacing as usize)).max(1);
        ...
        for (i, &card) in self.cards.iter().take(visible_cards).enumerate() {
            let is_selected = self.selected_idx.map_or(false, |idx| idx == i);
            ...
            CardView::new(card).selected(is_selected).render(card_area, buf);
        }
    }
}
...
impl Widget for TableView {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let visible_pairs = ((area.width as usize) / (pair_width as usize + spacing as usize)).max(1);
        ...
        for (i, (attack_card, defend_card)) in self.table_cards.iter().take(pairs_to_render).enumerate() {
