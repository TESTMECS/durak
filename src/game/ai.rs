...
impl AiStrategy for EasyStrategy {
    ...
    fn should_take_cards(&self, game_state: &GameState, player_idx: usize) -> bool {
        ...
        let trump_suit = game_state.trump_suit().unwrap_or_else(|| panic!("Trump suit should be set"));
        
        // Find the first undefended attack
        for (attack_card, _) in game_state.table_cards().iter().filter(|(_, defense)| defense.is_none()) {
            // Check if the player can defend against this card
            let can_defend = hand.iter().any(|card| card.can_beat(attack_card, trump_suit));
            
            if !can_defend {
                // If any single card can't be beaten, take all cards
                debug(format!("Easy AI ({}) cannot defend against {}, taking cards", player_idx, attack_card));
                return true;
            }
        }
        
        // If all cards can be beaten, still 80% chance to take cards (easy difficulty behavior)
        let random_take = rand::random::<f32>() < 0.8;
        if random_take {
            debug(format!("Easy AI ({}) randomly deciding to take cards", player_idx));
            return true;
        }
    ...
    fn make_attack_move(
        ...
        if !table_cards.is_empty() {
            ...
            for (idx, card) in hand.iter().enumerate() {
                if table_ranks.contains(&card.rank) {
                    debug(format!("Easy AI adding matching card {} to the attack", card));
        ...
        for (idx, card) in hand.iter().enumerate() {
            ...
            let is_trump = trump_suit.map_or(false, |trump| card.suit == trump);
            let card_value = if is_trump { 100 + card.rank as u8 } else { card.rank as u8 };
    ...
    fn make_defense_move(
        ...
        // Easy AI logic: simply use the lowest possible card that can defend
        // It doesn't save trumps strategically
        let player = &game_state.players()[player_idx];
        let hand = player.hand();
        let trump_suit = game_state.trump_suit().expect("Trump suit required for defense");
            ...
            .find(|(_, defense)| defense.is_none())
        {
            ...
            let trump_defenses: Vec<(usize, Card)> = hand
                ...
                .filter(|(_, card)| 
                    ...
                    (card.suit == trump_suit && attack_card.suit == trump_suit && card.rank > attack_card.rank)
                )
                .map(|(idx, &card)| (idx, card))
                .collect();
...
impl AiStrategy for MediumStrategy {
    ...
    fn should_take_cards(&self, game_state: &GameState, player_idx: usize) -> bool {
        ...
        let player = &game_state.players()[player_idx];
        let hand = player.hand();
        let trump_suit = game_state.trump_suit().expect("Trump suit required for defense decision");
        ...
        for attack_card in &undefended_attacks {
            ...
            if possible_defenses.is_empty() {
                debug(format!("Medium AI ({}) cannot defend against {}, taking cards", player_idx, attack_card));
                return true; // Cannot defend this card, must take all
            }
            
            // Check if defense requires a trump
            let requires_trump = possible_defenses
                .iter()
                .all(|card| card.suit == trump_suit);
        ...
        if high_trumps_needed > 0 {
            debug(format!("Medium AI ({}) taking cards to save high trumps", player_idx));
            return true;
        }
        ...
        if trump_cards_needed >= 2 {
            ...
            if random_take {
                debug(format!("Medium AI ({}) taking cards to save multiple trumps", player_idx));
                return true;
            }
        }
        
        if undefended_attacks.len() >= 4 {
            debug(format!("Medium AI ({}) taking cards due to too many attacks ({})", 
                player_idx, undefended_attacks.len()));
            return true;
        }
    ...
    fn make_attack_move(
        ...
        if !table_cards.is_empty() {
            ...
            if stop_adding {
                debug(format!("Medium AI ({}) decided to stop adding cards", player_idx));
                return Some(vec![]);
            }
            ...
            if !matching_non_trumps.is_empty() {
                ...
                if let Some((idx, card)) = matching_non_trumps
                    ...
                    .map(|(idx, card)| (*idx, *card))
                {
                    debug(format!("Medium AI adding non-trump card {} to attack", card));
                    return Some(vec![(idx, card)]);
                }
            }
            
            // If defender used a trump, medium AI might add a matching trump (30% chance)
            let defender_used_trump = table_cards
                .iter()
                .any(|(_, defense)| defense.map_or(false, |d| 
                    trump_suit.map_or(false, |t| d.suit == t)));
            ...
            if defender_used_trump {
                ...
                if add_trump {
                    ...
                    let matching_trumps: Vec<(usize, Card)> = hand
                        ...
                        .filter(|(_, card)| {
                            valid_ranks.contains(&card.rank) && 
                            trump_suit.map_or(false, |trump| card.suit == trump)
                        })
                        .map(|(idx, &card)| (idx, card))
                        .collect();
        ...
        let non_trump_pairs: Vec<(&Rank, &Vec<(usize, Card)>)> = rank_counts
            ...
            .filter(|(_, cards)| cards.len() >= 2 && 
                cards.iter().any(|(_, c)| trump_suit.map_or(true, |t| c.suit != t)))
            .collect();
        ...
        if !non_trump_pairs.is_empty() {
            if let Some((_, cards)) = non_trump_pairs.iter().min_by_key(|(rank, _)| *rank) {
                ...
                if let Some((idx, card)) = cards
                    .iter()
                    .filter(|(_, c)| trump_suit.map_or(true, |t| c.suit != t))
        ...
        let trump_cards: Vec<(usize, Card)> = hand
            .iter()
            .enumerate()
            .filter(|(_, card)| trump_suit.map_or(false, |t| card.suit == t))
            .map(|(idx, &card)| (idx, card))
            .collect();
            
        if trump_cards.len() > 1 {
            if let Some((idx, card)) = trump_cards.iter().min_by_key(|(_, c)| c.rank).map(|(i, c)| (*i, *c)) {
                debug(format!("Medium AI playing lowest trump (has multiple): {}", card));
                return Some(vec![(idx, card)]);
            }
        }
            ...
            .find(|(_, defense)| defense.is_none())
        {
            ...
            if !possible_passes.is_empty() {
                ...
                if pass_chance < 0.3 {
                    ...
                    let lowest_pass = possible_passes
                        .iter()
                        .min_by_key(|(_, card)| {
            ...
            if is_high_value {
                ...
                if use_trump_strategically {
                    ...
                    if !trump_defenses.is_empty() {
                        ...
                        let lowest_trump = trump_defenses
                            .iter()
                            .min_by_key(|(_, card)| card.rank);
                            
                        if let Some(&&(idx, card)) = lowest_trump {
                            debug(format!("Medium AI using trump {} to beat high value card {}", 
                                card, attacking_card));
                            return Some(vec![(idx, card)]);
                        }
                    }
            ...
            if !non_trump_defenses.is_empty() {
                ...
                if let Some(&&(idx, card)) = non_trump_defenses
                    .iter()
                    .min_by_key(|(_, card)| card.rank)
                {
            ...
            if let Some(&(idx, card)) = valid_defenses
                ...
                .min_by_key(|(_, c)| c.rank)
            {
                debug(format!("Medium AI forced to use trump {} (lowest available)", card));
                return Some(vec![(idx, card)]);
            }
...
impl AiStrategy for HardStrategy {
    ...
    fn should_take_cards(&self, game_state: &GameState, player_idx: usize) -> bool {
        ...
        for attack_card in &undefended_attacks {
            ...
            if defenses.is_empty() {
                debug(format!("Hard AI ({}) cannot defend against {}, must take", player_idx, attack_card));
        ...
        if is_endgame && high_trumps_used > 0 {
            ...
            let holding_last_high_trumps = high_trumps_used >= hand
                .iter()
                .filter(|card| card.suit == trump_suit && card.rank >= Rank::Jack)
                .count();
                
            if holding_last_high_trumps && high_trumps_played < 4 {
                debug(format!("Hard AI ({}) preserving last high trumps in endgame", player_idx));
                return true;
            }
        }
        ...
        if new_hand_size > hand_limit + 2 && valuable_cards_used < 2 {
            debug(format!("Hard AI ({}) avoiding taking too many cards ({})", player_idx, cards_to_take));
            return false;
        }
        ...
        let opponent_idx = if player_idx == defender_idx { attacker_idx } else { defender_idx };
        ...
        if is_endgame && opponent_card_count <= 2 && valuable_cards_used <= 1 {
            debug(format!("Hard AI ({}) defending aggressively against nearly-empty opponent", player_idx));
            return false;
        }
        ...
        if strategic_take {
            debug(format!("Hard AI ({}) strategically taking cards (value cards: {}, high trumps: {})", 
                player_idx, valuable_cards_used, high_trumps_used));
            return true;
        }
        
        debug(format!("Hard AI ({}) decides to defend (cards used: {})", player_idx, valuable_cards_used));
        false
    }
    ...
    fn make_attack_move(
        ...
        // Additional attack (not the first card)
        if !table_cards.is_empty() {
            ...
            // Check if defender used trumps, which indicates they may be low on certain suits
            let defender_used_trumps = table_cards
                .iter()
                .any(|(_, defense)| defense.map_or(false, |d| 
                    trump_suit.map_or(false, |t| d.suit == t)));
            ...
            if !probable_weak_ranks.is_empty() {
                ...
                if !matching_cards.is_empty() {
                    ...
                    if let Some(&(idx, card)) = matching_cards
                        .iter()
                        .min_by_key(|(_, c)| {
            ...
            if !matching_cards.is_empty() {
                ...
                if defender_used_trumps {
                    ...
                    if add_trump {
                        ...
                        if let Some(&&(idx, card)) = matching_trumps
                            ...
                            .min_by_key(|(_, c)| c.rank)
                        {
                            debug(format!("Hard AI strategically adding trump {} to pressure defender", card));
                            return Some(vec![(idx, card)]);
                        }
                    }
                }
                ...
                if !non_trump_matches.is_empty() {
                    ...
                    if let Some(&&(idx, card)) = non_trump_matches
                        .iter()
                        .min_by_key(|(_, c)| c.rank)
                    {
                ...
                if let Some(&(idx, card)) = matching_cards
                    ...
                    .min_by_key(|(_, c)| c.rank)
                {
                    debug(format!("Hard AI adding lowest matching card {} to attack", card));
                    return Some(vec![(idx, card)]);
                }
            }
            ...
            let easy_defense = table_cards
                .iter()
                .all(|(_, defense)| defense.is_some());
                
            if easy_defense || table_cards.len() >= 3 {
                let stop_chance = if easy_defense { 0.8 } else { 0.5 };
                
                if rand::random::<f32>() < stop_chance {
                    debug(format!("Hard AI strategically stops adding cards (easy defense: {})", easy_defense));
                    return Some(vec![]);
                }
            }
        ...
        // Initial attack strategy - varies based on game phase
        ...
        if is_endgame {
            ...
            if defender_hand_size <= 2 {
                ...
                if !forcing_cards.is_empty() {
                    ...
                    if let Some(&(idx, card)) = forcing_cards
                        .iter()
                        .min_by_key(|(_, c)| c.rank)
                    {
        ...
        if !pairs.is_empty() {
            ...
            if !non_high_trump_pairs.is_empty() {
                if let Some((_, cards)) = non_high_trump_pairs
                    .iter()
        ...
        if !non_trumps.is_empty() {
            ...
            if let Some(&(idx, card)) = non_trumps
                .iter()
                .min_by_key(|(_, c)| c.rank)
            {
        ...
        if let Some((idx, &card)) = hand
            .iter()
            .enumerate()
            .filter(|(_, c)| trump_suit.map_or(false, |t| c.suit == t))
            .min_by_key(|(_, c)| c.rank)
        {
        ...
        let (idx, &card) = hand
            ...
            .enumerate()
            .min_by_key(|(_, c)| c.rank)
            .unwrap(); // Safe because we checked for empty hand
    ...
    fn make_defense_move(
            ...
            .map(|(idx, (attack, _))| (idx, attack))
        {
            ...
            if !possible_passes.is_empty() {
                ...
                if possible_passes.len() > 0 && safe_passes.is_empty() {
                    // Find the lowest value pass card
                    if let Some(&(hand_idx, pass_card)) = possible_passes
                        .iter()
                        .min_by_key(|(_, card)| card.rank)
                    {
                        debug(format!("Hard AI forced to pass with {} as last resort", pass_card));
                        return Some(vec![(hand_idx, pass_card)]);
                    }
                }
            }
            ...
            if valid_defenses.is_empty() {
                ...
                if !possible_passes.is_empty() {
                    ...
                    debug(format!("Hard AI forced to pass with {} (no other defense)", pass_card));
                    return Some(vec![(hand_idx, pass_card)]);
                }
            ...
            if !non_trump_defenses.is_empty() {
                ...
                if let Some(&&(hand_idx, card)) = non_trump_defenses
                    .iter()
                    .min_by_key(|(_, card)| card.rank)
                {
            ...
            if !trump_defenses.is_empty() {
                ...
                if is_endgame {
                    ...
                    if !is_high_value_attack {
                        ...
                        if let Some(&&(hand_idx, card)) = low_trump_defense {
                            debug(format!("Hard AI using low trump {} to conserve high trumps", card));
                            return Some(vec![(hand_idx, card)]);
                        }
                    }
                }
                ...
                if let Some(&&(hand_idx, card)) = trump_defenses
                    .iter()
                    .min_by_key(|(_, card)| card.rank)
                {
